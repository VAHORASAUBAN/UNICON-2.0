...
class LoginRequiredMiddleware:
    ...
    def __call__(self, request):
        ...
        allowed_paths = ["/student/login/", "/logout/",
                         "/faculty/faculty_login/", "/admin/", "/faculty_logout_view/"]

        if not request.session.get("username") and request.path not in allowed_paths:
            # Redirect to login if not authenticated
            return render(request, "faculty/faculty_login.html", {"error": "Please log in first."})
